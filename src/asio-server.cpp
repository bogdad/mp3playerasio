...
class TcpConnection : public std::enable_shared_from_this<TcpConnection> {
  ...
  void send_date()
  {
    ...
    send([ptr]() { ptr->send_mp3(); },
        [](asio::error_code const&) { LOG(ERROR) << "send date error"; });
  }
    ...
    if (_file.send_chunk(_socket) == 0) {
      if (_file.is_all_sent()) {
        ...
        _socket.async_write_some(
            asio::null_buffers(),
            [ptr](asio::error_code const& error, size_t bytes_transferred) {
              ptr->send_mp3_inner();
            });
      }
    } else {
  ...
  send(absl::AnyInvocable<void() const>&& continuation,
      absl::AnyInvocable<void(asio::error_code const&) const>&& on_error)
  {
    auto ptr = shared_from_this();
    _socket.async_write_some(
        _write_buffer.data(), [this, ptr, on_error = std::move(on_error), continuation = std::move(continuation)](asio::error_code const& ec, size_t const bytes_transferred) mutable {
